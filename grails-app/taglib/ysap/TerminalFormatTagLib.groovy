...
class TerminalFormatTagLib {
    static namespace = "cf"

    def formatText = { attrs, body ->
        def style = attrs.style ?: "default"
        def color = attrs.color ?: "default"
        def special = attrs.special ?: "none"

        // Generate ANSI escape code based on style, color, and special parameters
        def ansiCode = generateAnsiCode(style, color, special)

        out << "${ansiCode}${body()}${ansiReset()}"
    }

    private String generateAnsiCode(String style, String color, String special) {
    ...
    private int getSpecialCode(String special) {
        switch (special) {
            case "blink":       return 5
